...
def estimate_metallicity(pixbin_path, ref_img_path=None, logMstar=10,
                         ...
                         nebular_metal_table='nebular_metallicity.npy'):
    '''
    ...
    nebular_metal_table : str
        Path to save the nebular metallicity table (numpy array).
    '''
    ...
    np.save(nebular_metal_table, np.array(Z_bin_arr))



def plot_metal_map(nebular_metal_table, pixbin_path, **kwargs):
    '''
    ...
    -----------
    nebular_metal_table : str
        Path to the nebular metallicity table (numpy array).
    pixbin_path : str
        Path to the pixel binning FITS file.
    '''
    ...
    binmap = fits.open(pixbin_path)[0].data
    nebular_metal = np.load(nebular_metal_table)
